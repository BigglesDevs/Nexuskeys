...
const Variants = {
  ...
  stock: (variantId) => db.prepare("SELECT COUNT(*) as c FROM keys WHERE variant_id=?").get(variantId).c,
};
...
const Keys = {
  ...
  deleteById: (id) => db.prepare("DELETE FROM keys WHERE id=?").run(id),
};
...
const Orders = {
  ...
  getRecent: (limit=50) => db.prepare("SELECT * FROM orders ORDER BY created_at DESC LIMIT ?").all(limit),
  getStats: () => db.prepare(`
