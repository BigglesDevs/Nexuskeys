...
const { Products, Variants, Keys, Orders, Tickets, Users } = require("../shared/db");
...
app.get("/api/products/:id", (req, res) => {
  ...
  res.json({ ...p, variants: Variants.getByProduct(p.id).map(v => ({ ...v, stock: Keys.stock(v.id) })) });
});
...
async function handleStripeWebhook(req, res) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // Claim key — permanently deletes it from DB
    const keyValue = Keys.claimAndDelete(order.variant_id);
    if (!keyValue) {
      Orders.fail(order.id);
      console.error(`No key available for order ${order.id}`);
...
app.listen(PORT, () => console.log(`NexusKeys API on port ${PORT}`));
